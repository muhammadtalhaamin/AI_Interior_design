...
function DesignType({selectedDesignType}) {
  ...
  return (
    <div className='mt-5'>
        <label className='text-gray-500'>Select your Interior Design Type</label>
        <div className='grid grid-cols-2 mt-3 md:grid-cols-3 lg:grid-cols-4 gap-5'>
            {Designs.map((design,index)=>(
                ...
                className={`
                h-[70px] rounded-md hover:scale-105 transition-all cursor-pointer ${design.name==selectedOption&&'border-2 border-purple-500 rounded-md p-1'}`
                }/>
                <h2>{design.name}</h2>
                </div>
            ))}
